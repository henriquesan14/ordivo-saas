    ...
    TimeProvider timeProvider) : ICommandHandler<RefreshPlatformSessionCommand, PlatformAuthDto>
{
    public async Task<Result<PlatformAuthDto>> Handle(RefreshPlatformSessionCommand command, CancellationToken ct)
    {
        ...
        if (session is null || session.SubjectType != AuthSubjectType.PlatformUser || !session.IsActive(now))
        ...
        var replacement = AuthSession.Create(
            user.Id, null, AuthSubjectType.PlatformUser, refreshToken.Hash, refreshToken.ExpiresAt);
