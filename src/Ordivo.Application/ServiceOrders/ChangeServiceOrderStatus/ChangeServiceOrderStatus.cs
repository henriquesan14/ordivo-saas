...
public sealed record ChangeServiceOrderStatusCommand(Guid ServiceOrderId, ServiceOrderStatus Status, string? Note = null) : ICommand<ServiceOrderDto>;
public sealed class ChangeServiceOrderStatusCommandHandler(IServiceOrderRepository orders, IUnitOfWork unitOfWork, IUserContext userContext, TimeProvider timeProvider) : ICommandHandler<ChangeServiceOrderStatusCommand, ServiceOrderDto>
{
    public async Task<Result<ServiceOrderDto>> Handle(ChangeServiceOrderStatusCommand command, CancellationToken ct)
    {
        ...
        try
        {
            order.ChangeStatus(command.Status, userContext.Name ?? userContext.Email ?? "Unknown", timeProvider.GetUtcNow(), command.Note);
